...
def merchant(user_id):
    ...
    try:
        with connect.cursor() as cursor:
            ...
            money = hero_info['money']
            exp += 2
            human_description = ['местный купец', 'какой-то подозрительный тип', 'сапожник', 'путешественник в потрепаном костюме']
            answer = f'На рынке вы увидели как {random.choice(human_description)} продаёт хорошую ездовую лошадь за 120 крон.' + '\n' + \
                     '+2 опыта' + '\n' + '~~~~~~~~' + '\n'
